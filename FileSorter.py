...
files = os.listdir() #Get files in folder
for i in files: #Sort files in specific folders
    ...
    if (i.lower().endswith(".exe")):                                                                 #PROGRAMS
        shutil.move(currentDir + "/" + i, programDir + "/" + i)
    if (i.lower().endswith(".mp3")  or i.lower().endswith(".waw")):                                    #AUDIO
        shutil.move(currentDir + "/" + i, audioDir + "/" + i)
    if (i.lower().endswith(".pdf") or i.lower().endswith(".txt")  or i.lower().endswith(".html")
        or i.lower().endswith(".docx") or i.lower().endswith(".odt") or i.lower().endswith(".doc") ):    #DOCUMENTS
        shutil.move(currentDir + "/" + i, documentDir + "/" + i)
    if (i.lower().endswith(".mp4") or i.lower().endswith(".wmw") or i.lower().endswith(".avi")):          # Video
        shutil.move(currentDir + "/" + i, programDir + "/" + i)
    if (i.lower().endswith(".zip") or i.lower().endswith(".rar") or i.lower().endswith(".jar")
        or i.lower().endswith(".gz") or i.lower().endswith(".iso")):                                     # ARCHIVE
        shutil.move(currentDir + "/" + i, archiveDir + "/" + i)
    if (i.lower().endswith(".torrent")):                                                               # torrents
